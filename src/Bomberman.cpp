...
// Last update Sun Jun  9 21:38:05 2013 laurent loukopoulos
//
...
namespace	Model
{
  ...
  void		Bomberman::update(gdl::GameClock const & gameClock, gdl::Input & input)
  {
    ...
    if (this->bonus_.remove(sx3, sy3) == true)
      {	
	this->speed3 += 10;
      }
    if (this->game != 0)
    ...
    if ((int)this->sx3 == (int)px1 && (int)this->sy3 == (int)py1)
      life_check1();
  }
  ...
  void		Bomberman::life_check3()
  {
    this->game = 1;
    std::cout<<"bot dead"<<std::endl;
    music_.Stop();
  }

  void		Bomberman::erase_map_trunk(int x1, int y1, int power)
  {
    int		i = 1, y , x, y2, x2 ,x3, y3;
	...
	x2 = -((((-this->move2_x) / 150 ) - this->map_.x) / 2);
	y2 = ((((-this->move2_y) / 150 ) + this->map_.y) / 2);
	x3 = -((((-this->posIa_x) / 150 ) - this->map_.x) / 2);
	y3 = ((((-this->posIa_y) / 150 ) + this->map_.y) / 2);
	music_.Stop();
	...
	if (this->map_.nb_players == "2")
	...
	else
	  {
	    if(x3 == (x1) && sy3 == y1)
	      this->life_check3();
	    if((x3 == (x1 + i) && y3 == y1) || (x3 == (x1 - i) && y3 == y1))
	      this->life_check3();
	    if((x3 == (x1) && y3 == y1 + i) || (x3 == (x1) && y3 == y1 - i))
	      this->life_check3();
	  }
